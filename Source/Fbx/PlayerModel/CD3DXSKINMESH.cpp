...
void CD3DXSKINMESH::DrawFrame( LPD3DXFRAME pFrameRoot)
{
	MYMESHCONTAINER *pMeshContainer;
	LPD3DXFRAME pFrame = pFrameRoot;

	pMeshContainer = (MYMESHCONTAINER*)pFrame->pMeshContainer;


	
	//	フレーム階層を精査し、メッシュが見つかった場合は　ロードを行い描画する
	//	これは動作確認用のため、こうしたもので本来であれば事前にやっておくべき事
	//	しかしながら、現在のサンプルでは、最後に見つかったメッシュに対してのみ
	//	メッシュ情報のロードが行われているため実質的にワンメッシュになっている
	...
	while (pMeshContainer != NULL)
	{
		...
		UINT offset = 0;
		m_pDeviceContext->IASetVertexBuffers(0, 1, &pMeshContainer->pVertexBuffer, &stride, &offset);
		...
		{
			//使用されていないマテリアル対策
			if(pMeshContainer->pD3D11Material[i].dwNumFace==0)
			{
				continue;
			}
			//インデックスバッファーをセット
			stride = sizeof( int );
			offset = 0;
			m_pDeviceContext->IASetIndexBuffer(pMeshContainer->ppIndexBuffer[i], DXGI_FORMAT_R32_UINT, 0 );

			for (int mat = 0; mat < 4; ++mat)
			{
				for (int mt = 0; mt < 4; ++mt)
				{
					m_View.m[mat][mt] = CameraManager::GetInstance().xmView.r[mat].m128_f32[mt];
					m_Proj.m[mat][mt] = CameraManager::GetInstance().xmProj.r[mat].m128_f32[mt];
				}
			}
			...
			{
				D3DXMATRIX tempRot,tempScale;
				D3DXMatrixTranslation(&Tran, m_Pos.x, m_Pos.y, m_Pos.z);
				D3DXMatrixRotationYawPitchRoll(&tempRot, m_Rot.x, m_Rot.y, m_Rot.z);
				D3DXMatrixScaling(&tempScale, m_Scale.x, m_Scale.y, m_Scale.z);
				
				m_World = tempScale * tempRot *Tran;
				SHADER_SKIN_GLOBAL1 sg;
				sg.mW= Tran;
				D3DXMatrixTranspose( &sg.mW, &sg.mW );
				sg.mWVP= Tran*m_View*m_Proj;
				D3DXMatrixTranspose( &sg.mWVP, &sg.mWVP );
				sg.vAmbient= pMeshContainer->pD3D11Material[i].Ka;
				sg.vDiffuse= pMeshContainer->pD3D11Material[i].Kd;
				sg.vSpecular= pMeshContainer->pD3D11Material[i].Ks;
			...
			//テクスチャーをシェーダーに渡す
			if(pMeshContainer->pD3D11Material[i].szTextureName[0] != NULL)
			{
				m_pDeviceContext->PSSetSamplers(0,1,&m_pSampleLinear);
				m_pDeviceContext->PSSetShaderResources(0,1,&pMeshContainer->pD3D11Material[i].pTexture);
			...
			//Draw
			m_pDeviceContext->DrawIndexed(pMeshContainer->pD3D11Material[i].dwNumFace*3,0,0);
		}
...
void CD3DXSKINMESH::LoadResource(LPD3DXFRAME pFrameRoot)
{
	MYMESHCONTAINER *pMeshContainer;
	LPD3DXFRAME pFrame = pFrameRoot;
	...
	while (pMeshContainer != NULL)
	{
		//	パーサーに処理したいメッシュをセットし、以降を処理をすすめる
		m_pD3dxMesh->m_pContainer = (MYMESHCONTAINER*)pMeshContainer;
		...
		int* piFaceBuffer = new int[m_dwNumFace * 3];//３頂点ポリゴンなので、1フェイス=3頂点(3インデックス)
													 ...
													 //頂点読み込み
		for (unsigned int i = 0; i<m_dwNumVert; i++)
		{
		...
		for (unsigned int i = 0; i<m_dwNumFace * 3; i++)
		{
...
//		m_pMaterial = new MY_SKINMATERIAL[m_dwNumMaterial];
		...
		pMeshContainer->pD3D11Material = new MY_SKINMATERIAL[m_dwNumMaterial];
		//マテリアルの数だけインデックスバッファーを作成
		//pIndexBuffer = new ID3D11Buffer*[m_dwNumMaterial];
		pMeshContainer->ppIndexBuffer = new ID3D11Buffer*[m_dwNumMaterial];
		
		for (unsigned int i = 0; i<m_dwNumMaterial; i++)
		{
			//環境光	
			pMeshContainer->pD3D11Material[i].Ka.x = m_pD3dxMesh->GetAmbient(i).y;
			pMeshContainer->pD3D11Material[i].Ka.y = m_pD3dxMesh->GetAmbient(i).z;
			pMeshContainer->pD3D11Material[i].Ka.z = m_pD3dxMesh->GetAmbient(i).w;
			pMeshContainer->pD3D11Material[i].Ka.w = m_pD3dxMesh->GetAmbient(i).x;
			//拡散反射光	
			pMeshContainer->pD3D11Material[i].Kd.x = m_pD3dxMesh->GetDiffuse(i).y;
			pMeshContainer->pD3D11Material[i].Kd.y = m_pD3dxMesh->GetDiffuse(i).z;
			pMeshContainer->pD3D11Material[i].Kd.z = m_pD3dxMesh->GetDiffuse(i).w;
			pMeshContainer->pD3D11Material[i].Kd.w = m_pD3dxMesh->GetDiffuse(i).x;
			//鏡面反射光
			pMeshContainer->pD3D11Material[i].Ks.x = m_pD3dxMesh->GetSpecular(i).y;
			pMeshContainer->pD3D11Material[i].Ks.y = m_pD3dxMesh->GetSpecular(i).z;
			pMeshContainer->pD3D11Material[i].Ks.z = m_pD3dxMesh->GetSpecular(i).w;
			pMeshContainer->pD3D11Material[i].Ks.w = m_pD3dxMesh->GetSpecular(i).x;

			D3DXVECTOR3 v(pMeshContainer->pD3D11Material[i].Ka.x, pMeshContainer->pD3D11Material[i].Ka.y, pMeshContainer->pD3D11Material[i].Ka.z);

			D3DXVECTOR3 vv(pMeshContainer->pD3D11Material[i].Ks.x, pMeshContainer->pD3D11Material[i].Ks.y, pMeshContainer->pD3D11Material[i].Ks.z);
			...
			if (name)
			{
				strcpy(pMeshContainer->pD3D11Material[i].szTextureName, name);
			}

			SAFE_RELEASE(pMeshContainer->pD3D11Material[i].pTexture);
			if (pMeshContainer->pD3D11Material[i].szTextureName[0] != 0 && FAILED(D3DX11CreateShaderResourceViewFromFileA(m_pDevice, "Resource/Model/Player.jpg", NULL, NULL, &pMeshContainer->pD3D11Material[i].pTexture, NULL)))//絶対パスファイル名は、まず失敗すると思うが、、、
			{
			...
			for (unsigned int k = 0; k<m_dwNumFace; k++)
			{
			...
			if (iCount>0) CreateIndexBuffer(iCount * sizeof(int), pIndex, &pMeshContainer->ppIndexBuffer[i]);
			delete[] pIndex;
			pMeshContainer->pD3D11Material[i].dwNumFace = iCount / 3;//そのマテリアル内のポリゴン数
		...
		m_pDevice->CreateBuffer(&bd, &InitData, &pMeshContainer->pVertexBuffer);
	...
	if (pFrame->pFrameSibling != NULL)
	{
		LoadResource(pFrame->pFrameSibling);
	}

	if (pFrame->pFrameFirstChild != NULL)
	{
		LoadResource(pFrame->pFrameFirstChild);
	}
}
...
DWORD D3DXPARSER::GetBoneVerticesIndices(int iBoneIndex,int iIndexInGroup)
{
	...
	DWORD retvalue = pVerts[iIndexInGroup];
	delete[] pVerts;
	delete[] pWights;
...
double D3DXPARSER::GetBoneVerticesWeights(int iBoneIndex,int iIndexInGroup)
{
	...
	double retvalue = pWights[iIndexInGroup];
	delete[] pVerts;
	delete[] pWights;
...
CD3DXSKINMESH::CD3DXSKINMESH()
{
	...
	m_Scale=D3DXVECTOR3(0.12f,0.12f,0.12f);
...
HRESULT CD3DXSKINMESH::Init(CD3DXSKINMESH_INIT* si)
{
	...
	//ブロブからバーテックスシェーダー作成
	if(FAILED(D3DX11CompileFromFile(L"Resource/Hlsl/Geometry_Material_Texture_Skin.hlsl",NULL,NULL,"VSSkin","vs_5_0",0,0,NULL,&pCompiledShader,&pErrors,NULL)))
	{
		int size=(int)pErrors->GetBufferSize();
	...
	D3D11_INPUT_ELEMENT_DESC layout[] =
	{
		...
		{ "BONE_INDEX", 0, DXGI_FORMAT_R32G32B32A32_UINT, 0, 36, D3D11_INPUT_PER_VERTEX_DATA, 0 },
		{ "BONE_WEIGHT", 0, DXGI_FORMAT_R32G32B32A32_FLOAT, 0, 52, D3D11_INPUT_PER_VERTEX_DATA, 0 },
	};
	...
	//ブロブからピクセルシェーダー作成
	if(FAILED(D3DX11CompileFromFile(L"Resource/Hlsl/Geometry_Material_Texture_Skin.hlsl",NULL,NULL,"PSSkin","ps_5_0",0,0,NULL,&pCompiledShader,&pErrors,NULL)))
	{
		MessageBox(0,L"hlsl読み込み失敗",NULL,MB_OK);
		return E_FAIL;
	}
	...
	for(i=0;i<iNumBone;i++)
	{
		...
		for(k=0;k<iNumIndex;k++)
		{
			...
			if(flag==false)
			{
				...
				pvVB[ piIndex[k] ].bBoneWeight[3]=(float)pdWeight[k];
				break;
			}
...
HRESULT CD3DXSKINMESH::CreateFromX(CHAR* szFileName)
{
	...
	//	事前にリソースを準備する
	LoadResource(m_pD3dxMesh->m_pFrameRoot);
...
void CD3DXSKINMESH::Render()
{
	for (int mm = 0; mm < 4; ++mm)
	{
		for (int mt = 0; mt < 4; ++mt)
		{
			m_View.m[mm][mt] = CameraManager::GetInstance().xmView.r[mm].m128_f32[mt];
		}
	}
	...
	{
		m_pD3dxMesh->m_pAnimController->AdvanceTime(0.01667f,NULL);
	}
